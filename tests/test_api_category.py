...
def test_get_category_scalar(test_app,test_client,db_session,category1):
    res = test_client.get('/api_v1/categories/1')
    json_data = json.loads(res.data)
    assert res.status_code, 200
    assert 'category1' == json_data.get("name")

def test_get_category_collection(test_app,test_client,db_session,category1,category2):
    res = test_client.get('/api_v1/categories')
    json_data = json.loads(res.data)
    assert res.status_code, 200
    assert 'category2' == json_data.get("name")
